...
class TCID:
	...
	def validate(self):
		for i, x in enumerate(self.iterable):
			if i == 0 and x is not None: assert re.match('^[0-9]$', x), 'Invalid class id "{}" (1st field)'.format(x)
			elif i == 1 and x is not None: assert re.match('^[A-Z]$', x), 'Invalid subclass id "{}" (2nd field)'.format(x)
			elif i == 2 and x is not None: assert re.match('^[0-9]+$', x), 'Invalid family id "{}" (3rd field)'.format(x)
			elif i == 3 and x is not None: assert re.match('^[0-9]+$', x), 'Invalid subfamily id "{}" (4th field)'.format(x)
			elif i == 4 and x is not None: assert re.match('^[0-9]+$', x), 'Invalid system id "{}" (5th field)'.format(x)
			elif i == 5 and x is not None: assert True, 'Invalid external accession "{}" (6th field)'.format(x)
